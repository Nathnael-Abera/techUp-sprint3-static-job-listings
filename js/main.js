...
function getJobListingHTML(jobData, filterTags = []) {
    ...
    let jobListingHTML = `
        <div class="jobs__item ${jobData.postedAt === '1d ago' ? 'border-left' : ''}">
            <div class="jobs__column jobs__column--left">
                <img src="${jobData.logo}" alt="${jobData.company}" class="jobs__img" />
                <div class="jobs__info">
                    <span class="jobs__company">${jobData.company} ${jobData.postedAt === '2d ago'||jobData.postedAt === '1d ago' ? '<span class="jobs__new">NEW!</span>' : ''}${ jobData.postedAt === '1d ago'? '<span class="jobs__featured">FEATURED</span>' : ''} </span>
